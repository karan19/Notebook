...
export function SideMargin({ notes, onNotesChange, isVisible }: SideMarginProps) {
    ...
    return (
        <motion.div
            ...
            className="relative h-full side-margin-gutter cursor-crosshair group/margin overflow-hidden pointer-events-auto border-l border-gray-100 dark:border-gray-800/50"
            onClick={addNote}
            title="Click to add a margin note"
        >
            <div className="w-[240px]"> {/* Fixed width inner container to prevent squishing text */}
                {/* Margin Label */}
                        ...
                        >
                            {editingId === note.id ? (
                                <textarea
                                    ...
                                    className="sticky-note-textarea min-h-[60px]"
